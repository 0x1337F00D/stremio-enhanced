...
`;
  ...
  async function checkSettings() {
    ...
    Settings_default.addButton("Import Theme", "openthemesfolderBtn", SELECTORS.THEMES_CATEGORY);
    Settings_default.addButton("Import Plugin", "openpluginsfolderBtn", SELECTORS.PLUGINS_CATEGORY);
    Helpers_default.waitForElm("#openthemesfolderBtn").then(() => {
        ...
        try {
          ...
          if (result.files.length > 0) {
            ...
            if (file.data && file.name) {
              ...
              await PlatformManager.current.writeFile((0, import_path2.join)(Properties_default.themesPath, file.name), decodedData);
              location.reload();
            }
          }
        } catch (err) {
          logger_browser_default.error("Failed to pick theme file: " + err);
        }
      });
    });
    Helpers_default.waitForElm("#openpluginsfolderBtn").then(() => {
      document.getElementById("openpluginsfolderBtn")?.addEventListener("click", async () => {
        try {
          ...
          if (result.files.length > 0) {
            ...
            if (file.data && file.name) {
              const decodedData = decodeURIComponent(escape(atob(file.data)));
              await PlatformManager.current.writeFile((0, import_path2.join)(Properties_default.pluginsPath, file.name), decodedData);
              location.reload();
            }
          }
        } catch (err) {
          logger_browser_default.error("Failed to pick plugin file: " + err);
        }
      });
    });
    writeAbout();
